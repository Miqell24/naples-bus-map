...
async function init() {
  ...
  const PALETTE = [
    ...
    // ANM official line colors (uppercase — exactly as the pipeline emits them
    // from routes.txt; a color missing here degrades to the darken() fallback
    // icon, so keep the list in sync)
    ['#F8E414', '#887D0B'], // L1
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      a.download = `naples-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `naples-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  // NOT fitBounds(meta.bbox): a handful of ANM lines run out to Pozzuoli and
  // the Campi Flegrei, so fitting the data bbox would open on the caldera with
  // the city as a small core. Open on Naples itself — the arc from Fuorigrotta
  // through the centre to San Giovanni, with the Vomero above it.
  map.jumpTo({ center: [14.2530, 40.8480], zoom: 12.1 });
}
