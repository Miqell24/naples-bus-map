...
// Naples: ONE ANM feed (anm.it/google/google-transit.zip) split by route_type
// into buses+trolleybuses (3, 11) on the road graph and everything on rails
// (0 tram, 1 metro, 7 funicular) on the rail graph. Trolleybuses ride the bus
// network in green per-line colors; metro L1/L2/L6 and the funiculars F1/F3/F4
// keep the official colors straight from routes.txt and trigger the engine's
// metro treatment (wide ribbon, station discs, always-on names).
// Usage: node pipeline/build.mjs [--all | lines...] [--tram all|412,L1]
...
// Naples numbers its rapid transit L1/L2/L6 (metro) and F1/F3/F4 (funiculars).
// Both get the engine's metro treatment — wide translucent ribbon, station
// discs, names that never yield — so ONE predicate decides it everywhere the
...
const nameDict = (() => {
  const docs = [];
  for (const f of ['data/osm/naples.json', 'data/osm/naples-names.json']) {
    const q = join(ROOT, f);
    if (!existsSync(q)) continue;
    try { docs.push(JSON.parse(readFileSync(q, 'utf8'))); } catch { /* unreadable extract: skip */ }
  }
  const d = buildNameDict(docs);
  console.log(`Italian names: dictionary of ${d.size} word forms from ${docs.length} OSM extract(s)`);
  return d;
})();
...
// ONE feed, FOUR cfgs: routeTypes splits the ANM bundle per mode (3 = bus,
...
// four different kinds of track and they must NOT share a graph: a metro shape
// often passes closer to the tram tracks or the mainline above the tunnel than
// to its own axis, and a mixed graph lures Viterbi onto disconnected rails.
// railKeep filters the OSM ways per cfg before building, and Linea 2 gets its
// own cfg because it is not a metro at all — it is Trenitalia's Passante,
// running on the national network that ANM merely publishes with its own feed.
...
const railCfg = (label, railKeep, routeTypes, keep, railFilter) => ({
  ...
  feeds: [
    { tag: 'anm', dir: 'data/gtfs', mapKey: (sn) => (!keep || keep(sn) ? sn : null), routeTypes, titleCase: true },
  ],
});
...
async function processMode(cfg) {
  ...
  for (const feed of cfg.feeds) {
    ...
    // ANM shouts every stop name — the dictionary built from OSM decides the
    // case word by word and puts the accents back (lib/italian.mjs)
    const titleCase = (s) => italianTitleCase(s, nameDict);
    ...
    // trim the shape to the passenger stretch: TPBI shapes overshoot the
  ...
  const metaLines = [...new Set(reps.map((r) => r.line))].sort(numSort).map((L) => ({
    line: L,
    mode: cfg.mode,
    // Which network runs it. Seven cfgs share this map — ANM's buses and trams,
    // the two metro tubes, Trenitalia's Linea 2, the funiculars and EAV's
    // railways — and several of them number from 1. The panel groups by this,
    // or the cloud reads "… L1 L6 L2 F1 F3 F4 1 1 2 4 5 5 …" with no way to
    // tell an EAV train from a tram.
    op: cfg.label,
    color: colorOf([L]),
